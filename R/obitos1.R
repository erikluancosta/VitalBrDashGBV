...
df_obitos <- df_linkada_fem_2019_2021_2 |>
  filter(sg_sexo == "F") |> 
  dplyr::mutate(
...
load("dados/icd_map_res.Rdata")

df_obitos <- df_obitos |> left_join(icd_map_res, by = c("cd_causabas" = "ICD_10"))


obitos_ui <- function(id) {
  ns <- NS(id)
  ...
  tagList(
                 ...
                 column(12,
                        wellPanel(
                          selectInput(inputId = ns("filtro_grupo"),
                                      ...
                                      multiple = FALSE,
                                      choices = c(
                                        "i. Mulheres com registro de óbito no SIM e/ou com Boletim de Ocorrência letal e que têm notificação de violência no SINAN" = "g1o", 
                                        "ii. Mulheres com registro de óbito no SIM e/ou Boletim de Ocorrência letal que possuem Boletim de Ocorrência por violência (não letal), mas sem registro de notificação no SINAN" = "g2o", 
                                        "iii. Mulheres que tiveram seu óbito registrado no SIM e têm registro de Boletim de Ocorrência letal" = "g3o"
                                      ),
                                      selected = "i. Mulheres com registro de óbito no SIM e/ou com Boletim de Ocorrência letal e que têm notificação de violência no SINAN")
                        ))
               )
             )),
    fluidRow(
    ...
    fluidRow(
      box(
        title = "Informações do SINAN para mulheres com desfecho em óbito",
        width = 12,
        status = "secondary",
        maximizable = TRUE,
        closable = FALSE,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$raca_cor_graf <- renderPlotly({
        ...
        cores <- setNames(rep("#1f1c86", length(racas_ordenadas)), racas_ordenadas)
      ...
      output$sinan <- renderDataTable({
        tryCatch({
          ...
          tabela_sinan <- df_obitos |>
            filter(grupo_m_ob %in% input$filtro_grupo) |>
      ...
      output$causa_ob <- renderPlotly({
        ...
        b <- ggplot(a, aes(
          x = reorder(causa_resumida, `%`), y = `%`, 
          text = paste("Causa resumida: ", causa_resumida, "\nPorcentagem: ", round(`%`, 2),"%", "\nRegistros: ", n)
        )) +
          geom_col(width = 0.001, fill = "#1f1c86", color = "#1f1c86") +
          geom_point(color = "#1f1c86") +
          theme_minimal() +
          labs(
